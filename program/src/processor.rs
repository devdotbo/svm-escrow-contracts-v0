...
pub fn process_instruction(
    ...
    match instruction {
        ...
        EscrowInstruction::Cancel => process_cancel(program_id, accounts, false),
        EscrowInstruction::PublicCancel => process_cancel(program_id, accounts, true),
        EscrowInstruction::PublicWithdraw { secret, proof } => {
            process_public_withdraw(program_id, accounts, secret, proof)
        }
        EscrowInstruction::RescueFunds => process_rescue_funds(program_id, accounts),
    }
}
...
fn process_withdraw(
    ...
    if escrow.merkle_root != [0u8; 32] {
        ...
        escrow_mut.filled_index = escrow_mut
            .filled_index
            .checked_add(1)
            .ok_or(EscrowError::Overflow)?;
    }
...
/// 5. [] Clock sysvar
/// 6. [] System program
fn process_cancel(program_id: &Pubkey, accounts: &[AccountInfo], is_public: bool) -> ProgramResult {
    let account_info_iter = &mut accounts.iter();
    ...
    // Check timelocks
    let exclusive_cancel_time =
        escrow.deployed_at + escrow.timelocks[Escrow::TL_DST_EXCLUSIVE_CANCEL];
...
/// 4. [] Clock sysvar
/// 5. [] System program
fn process_rescue_funds(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    let account_info_iter = &mut accounts.iter();
