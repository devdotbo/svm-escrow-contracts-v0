...
async fn test_cancel_by_resolver() {
    ...
    let mut cancel_transaction = Transaction::new_with_payer(
        &[cancel_ix],
        Some(&payer.pubkey()),
    );
    ...
    assert!(escrow_account_after.is_none(), "Escrow account should be closed after cancel");
    
    // Verify tokens returned to resolver
    let resolver_token_after = banks_client.get_account(resolver_token_account).await.unwrap();
    assert!(resolver_token_after.is_some(), "Resolver token account should exist");
...
async fn test_public_cancel() {
    ...
    let mut public_cancel_transaction = Transaction::new_with_payer(
        &[public_cancel_ix],
        Some(&stranger.pubkey()),
    );
    public_cancel_transaction.sign(&[&stranger], recent_blockhash);
    
    let cancel_result = banks_client.process_transaction(public_cancel_transaction).await;
    assert!(cancel_result.is_ok(), "Public cancel failed: {:?}", cancel_result);
    
    // Verify escrow was closed
    let escrow_account_after = banks_client.get_account(escrow_pda).await.unwrap();
    assert!(escrow_account_after.is_none(), "Escrow account should be closed after public cancel");
