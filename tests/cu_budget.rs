...
async fn test_cu_budget_create() {
    ...
    assert!(result.is_ok(), "CreateDstEscrow failed within {} CU budget", MAX_CU_TARGET);
    
    println!("✓ CreateDstEscrow completed within {} CU budget", MAX_CU_TARGET);
}
...
async fn test_cu_budget_withdraw_merkle() {
    ...
    assert!(result.is_ok(), "Withdraw with 32-level proof failed within {} CU budget", MAX_CU_TARGET);
    
    println!("✓ Withdraw with 32-level Merkle proof completed within {} CU budget", MAX_CU_TARGET);
}
...
async fn test_cu_budget_all_instructions() {
    ...
    for (ix_name, expected_pass) in instructions {
        if expected_pass {
            println!("  ✓ {} - Expected to pass within {} CU", ix_name, MAX_CU_TARGET);
        } else {
            println!("  ✗ {} - May exceed {} CU limit", ix_name, MAX_CU_TARGET);
        }
    }
    
    println!("\nAll critical operations designed to fit within {} CU budget", MAX_CU_TARGET);
